...
export const saveExercise = async ({ method, id, body }: FormExerciseParams): Promise<{ msg: string }> => {
...
export const getExerciseCategories = async (): Promise<ExerciseCategoryModel[]> => {
...
export const deleteExerciseCategory = async (id: string): Promise<{ msg: string }> => {
	const url = `/selections/exercise-categories/${id}`;
	return await apiClient<{ msg: string }>({ url: url, method: "DELETE" });
};

export const downloadExerciseVideo = async (id: number): Promise<Blob> => {
	const url = `/exercises/${id}/videos/download`;
	return await apiClient<Blob>({ url: url, method: "GET", responseType: "blob" });
};
