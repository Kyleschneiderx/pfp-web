...
export const validateDayForm = ({
	...
	const isNameExists = (arr: PfPlanDailies[], name: string, selectedDay: PfPlanDailies | null): boolean => {
		const foundItem = arr.find((obj) => obj.name.toLowerCase() === name.toLowerCase());
	...
	if (exercises.length === 0) {
		...
		exercises.forEach((e) => {
			...
			if (!restErrorAdded && (!e.rest || e.rest <= 0)) {
				errors.push({
					fieldName: "rest",
					message: "No. of rest should be greater than zero.",
				});
				restErrorAdded = true;
			}
		});
