...
import type { CategoryOptionsModel, EducationModel } from "@/app/models/education_model";
...
const PfPlanDropdownList = dynamic(() => import("@/app/components/education/pfplan-dropdown-list"), { ssr: false });
...
export default function EducationForm({ action = "Create", education }: Props) {
	...
	useEffect(() => {
		if (action === "Edit" && education) {
			...
			if (education?.reference_pf_plan_id) {
				const foundItem = pfplanOptions.find((el) => el.value === education.reference_pf_plan_id?.toString());
	...
	const handleDeleteConfirm = async () => {
		if (!isProcessing && education && action === "Edit") {
			try {
				setIsProcessing(true);
	...
	return (
		<>
				...
				<div className="flex items-center mb-4 sm:mb-7">
					<div>
						...
						<p className="text-sm text-neutral-600">
							{action === "Create" ? CREATE_EDUCATION_DESCRIPTION : UPDATE_DESCRIPTION}
						</p>
					</div>
					...
					<div className="space-y-4 w-full sm:w-auto">
						...
						<Card className="sm:w-[592px] sm:mr-5 space-y-3">
							...
							<div>
								...
								<PfPlanDropdownList value={pfplanRef} setValue={setPfplanRef} getOptions={handleGetPfPlanOptions} />
							</div>
							<div>
								<Label label="Content" />
								<TipTapEditor
						...
						<div className="sm:hidden order-last flex flex-col w-full mt-6 space-y-3">
							<div className="flex w-full space-x-3">
								...
								<div className="flex-1">
									<Button label="Save as Draft" secondary onClick={onDraft} className="w-full" />
								</div>
							</div>
			...
			<ConfirmModal
				title={`Are you sure you want to ${action === "Create" ? "create this education?" : "save this changes?"} `}
