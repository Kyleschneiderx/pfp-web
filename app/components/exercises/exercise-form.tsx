...
import { CategoryOptionsModel, ExerciseModel } from "@/app/models/exercise_model";
...
const CreateCategoryModal = dynamic(() => import("@/app/components/exercises/create-category-modal"), { ssr: false });
const CategoryListModal = dynamic(() => import("@/app/components/exercises/category-list-modal"), { ssr: false });
const ModalRename = dynamic(() => import("@/app/components/elements/ModalRename"), { ssr: false });
...
export default function ExerciseForm({ action = "Create", exercise }: Props) {
	...
	const [isCategoryProcessing, setIsCategoryProcessing] = useState<boolean>(false);

	const [modalOpen, setModalOpen] = useState(false);
	const [deleteModalOpen, setDeleteModalOpen] = useState(false);
	const [addCategoryModalOpen, setAddCategoryModalOpen] = useState(false);
	const [categoriesModalOpen, setCategoriesModalOpen] = useState(false);
	const [categoryConfirmModalOpen, setCategoryConfirmModalOpen] = useState(false);
	...
	const getCategories = async () => {
		try {
			...
			const apiError = error as ErrorModel;
			const errorMessage = apiError.msg || "Failed to fetch exercise categories";
	...
	return (
		<>
			<div className="flex items-center mb-5 sm:mb-7">
				<div>
					...
					<p className="text-sm text-neutral-600">
						{action === "Create" ? CREATE_EXERCISE_DESCRIPTION : UPDATE_DESCRIPTION}
					</p>
				</div>
			...
			<div className="flex flex-col sm:flex-row mt-3 sm:mt-8">
				<div className="w-full sm:w-[636px] sm:p-5 space-y-4 sm:mr-6 mb-4 sm:mb-0 z-10 rounded-lg sm:bg-white sm:drop-shadow-center">
					...
					<div>
						<div className="flex justify-between">
							...
							<span className="text-primary-500 cursor-pointer" onClick={() => setAddCategoryModalOpen(true)}>
								+ Add a new category
							</span>
						</div>
						<SelectCmp
				...
				<CreateCategoryModal isOpen={addCategoryModalOpen} onClose={handleCloseModal} onAddClick={handleAddCat} />
				<CategoryListModal
				...
				/>
				<ConfirmModal
					title={`Are you sure you want to ${action === "Create" ? "create this exercise?" : "save this changes?"} `}
				...
				<ConfirmModal
					title={`Are you sure you want to ${isDeleteCat ? "delete this" : "create this new"} category?`}
					subTitle={CONFIRM_DELETE_DESCRIPTION}
