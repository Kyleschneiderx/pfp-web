...
const ExercisePanel = dynamic(() => import("@/app/components/workouts/exercise-panel"), { ssr: false });
...
export default function WorkoutForm({ action = "Create", workout }: Props) {
	...
	const [editInfo, setEditInfo] = useState<boolean>(action === "Create" ? true : false);
	...
	return (
		<>
			<div className="flex items-center mb-4 sm:mb-7">
				<div>
					...
					<p className="text-sm text-neutral-600">
						{action === "Create" ? CREATE_WORKOUT_DESCRIPTION : UPDATE_DESCRIPTION}
					</p>
				</div>
					...
					<div className="flex w-full items-center space-x-4 mt-2 sm:mt-0">
						{editInfo && (
							<SelectCmp
								...
								value={type}
								invalid={false}
								onChange={(e) => setType(e as SelectOptionsModel)}
								placeholder="Select"
				...
				<div className="mt-5 sm:mt-6 border-l-4 border-primary-500 pl-2 sm:pl-4">
					<div className="flex sm:space-x-4 space-y-2 sm:space-y-0 flex-col sm:flex-row">
						<div className="flex space-x-2 sm:space-x-4 items-center">
							<div
								className={clsx(
									...
									type?.label === "Premium" ? "text-[#D5A215] bg-[#D5A21514]" : "text-primary-900 bg-secondary-100",
								)}
							>
								{type?.label === "Premium" && (
									<Image
						...
						<div className="flex sm:space-x-4 items-center">
							<div onClick={() => setEditInfo(true)} className="cursor-pointer mx-2 sm:mx-0">
								<PencilIcon />
							</div>
							<StatusBadge label={workout ? workout.status.value : "Draft"} />
						</div>
							...
							{(provided) => (
								<div className="space-y-6 mt-6" ref={provided.innerRef} {...provided.droppableProps}>
									{exercises.map((item, index) => (
										<Draggable key={"key" + item.id} draggableId={item.id.toString()} index={index}>
											{(provided, snapshot) => (
												<div
												...
												>
													...
													<CircleX className="text-error-600" onClick={() => onRemoveExercise(item.id)} />
												</div>
											)}
										</Draggable>
									))}
					...
					{exercises.length === 0 && (
						<div className="flex flex-col justify-center text-center items-center h-full sm:mt-0">
							<p className="text-neutral-400 mb-3">
								Add exercise from the exercise panel to begin creating your workout
							</p>
			...
			<ConfirmModal
				title={`Are you sure you want to ${action === "Create" ? "create this workout?" : "save this changes?"} `}
