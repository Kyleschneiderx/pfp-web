...
export default function ActionMenuMobile({ onDeleteClick, onDuplicateClick, onRenameClick, onSendInviteClick }: Props) {
	const { patient, exercise, workout, pfPlan, education, editUrl, isOpen, setIsOpen } = useActionMenuStore();
	...
	const itemClass = "block w-full py-2 hover:bg-primary-100 hover:text-primary-500 cursor-pointer";

	return (
		<div className="sm:hidden">
			{/* Background overlay */}
			{isOpen && <div className="fixed inset-0 bg-black opacity-50 z-20" onClick={() => setIsOpen(false)} />}
			{/* Action menu */}
			<div
				className={clsx(
			...
			>
				<div className="bg-white p-5 font-semibold rounded-t-2xl rounded-b-none">
					<p className="text-xl text-center mb-2">
						{patient?.user_profile?.name || exercise?.name || workout?.name || pfPlan?.name || education?.title}
					</p>
					<ul>
						<li>
							<Link href={editUrl} className={itemClass} onClick={() => setIsOpen(false)}>
								<span>Edit</span>
							</Link>
						</li>
						{onRenameClick && (
