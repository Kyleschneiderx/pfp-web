	...
	return (
		<aside className="min-w-[245px] shadow-xl flex flex-col h-full overflow-auto">
			...
			<nav className="flex-1">
				{NAVIGATIONS.filter((item) => item.permissions === undefined || hasPermission(item.permissions)).map(
					(item, index) => {
						return (
							<div key={index}>
								{item?.subItems?.length ? (
									<details className="group">
										...
										<div className="bg-gray-50/50">
											{item.subItems
												.filter((subItem) => subItem.permissions === undefined || hasPermission(subItem.permissions))
